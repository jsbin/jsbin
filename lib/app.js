...
(function () {
  ...
  if (process.env.HOST) {
    options.url.host = process.env.HOST;
  }

  var ssl_env = String(process.env.JSBIN_SSL);
  if (ssl_env.toLowerCase() == 'true') {
    options.url.ssl = true;
  } else if (ssl_env.toLowerCase() == 'false') {
    options.url.ssl = false;
  }
