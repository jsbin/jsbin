...
module.exports = Observable.extend({
   ...
   */
  github: passport.authenticate('github', { scope: ['user:email', 'gist'] }),
  ...
  githubCallback: function(req, res, next) {
    ...
    this.models.user.load(githubUser.profile.username, function (err, user) {
      if (err) return next(err);

      // If user was found but current user is not signed in, don't allow them
      // to connect github.
      if (user && !req.session.user) {
        // If the user is linked to github already then update the token and
        // log them in
        if (user.github_token) {
          user.github_token = githubUser.access_token;
          return this.models.user.updateGithubToken(user.name, user.github_token, function (err) {
            if (err) return next(err);
        ...
        // Don't allow them to connect – let them know there's already a user
        // with that account. Make them connect the new one.
        res.flash(res.flash.ERROR, 'A user with that username already exists. Please login as that user to link the accounts.');
        return res.redirect('/');
      }
      // If the current user is signed in then link the accounts.
      if (req.session.user) {
        user.github_token = githubUser.access_token;
        ...
        return this.models.user.updateGithubToken(user.name, user.github_token, function (err) {
          ...
          res.flash(res.flash.NOTIFICATION, 'Your JS Bin account ('+user.name+') has been linked to you GitHub account ('+githubUser.profile.username+'). Nice.');
          return res.redirect('/');
        }.bind(this));
      }
      // If no user found and they're not signed in then this is a new user,
