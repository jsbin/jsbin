...
module.exports = Observable.extend({
  ...
  jsbin: function (bin, options) {
    ...
    options.metadata = _.pick.apply(_, [options.metadata].concat('archive avatar created last_login last_updated name pro summary updated visibility'.split(' ')));
  ...
  formatPreview: function (bin, options, fn) {
    ...
    function onAnalyticsComplete(err, analytics) {
      ...
      if (!options.silent && bin.streaming_key) { // jshint ignore:line
  ...
  delete: function (req, res, next) {
    ...
    // now archive the bin - FIXME https://github.com/jsbin/jsbin/issues/1056
