...
module.exports = utils.inherit(Object, {
  ...
  updateUserGithubToken: function (id, token, fn) {
    var now = new Date();
    this.connection.query(templates.updateUserGithubToken, [token, now, id], function (err) {
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  getUserBinCount: function (id, fn) {
    ...
    this.connection.query(sql, values, function (err, result) {
      ...
      fn(null, { found: !!result.length, total: result.length ? result[0].total === 1 : 0 });
    });
  }
});
