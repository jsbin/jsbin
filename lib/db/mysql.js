...
/*    if (typeof this.options.charset === 'string') {
      ...
      this.connection.query('SET NAMES ?', params, fn);
    } else {*/
      fn();
//    }
  },
  ...
  setBinOwner: function (params, fn) {
    var sql = templates.setBinOwner,
        values = [params.name, params.url, params.revision, new Date(), params.summary, params.html, params.css, params.javascript, params.visibility];
  ...
  getBinMetadata: function(bin, fn) {
    ...
    this.connection.query(sql, [bin.url, bin.revision], function(err, result) {
      ...
      fn(null, result[0] ? result[0] : false);
    });           
  },
  ...
  setProAccount: function(id, pro, fn){
    var now = new Date();
    this.connection.query(templates.setProAccount, [pro, now, id], function (err) {
      if (err) {
        return fn(err);
      }
      fn(null);
