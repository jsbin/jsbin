...
module.exports = Observable.extend({
  ...
  testQuietAccess: function (req, res, next) {
    // checks whether the user who created the bin is registered and email is
    // verified, if not, it'll redirect without /quiet (chrome'd version of jsbin)

    // check if the quiet flag was used at all
    var quiet = !!req.param('quiet'),
        helpers = this.helpers;

    var noquiet = function () {
      res.redirect(301, helpers.url(req.bin.url + '/' + req.bin.revision));
    };
    ...
    if (req.bin) {
      this.models.user.owner(req.bin, function (err, result) {
        ...
        if (user.verified || user.github_id) {
          return next();
        } else {
          return noquiet();
        }
  ...
  getBinPreview: function (req, res, next) {
    ...
    this.protectVisibility(req.session.user, req.bin, function(err, bin) {
      new Promise(function (resolve, reject) {
        ...
        this.formatPreview(bin, req, options, function (err, formatted) {
          if (err) {
            next(err);
          }
        ...
        res.send(501, 'Failed to get bin preview');
      ...
      this.loadFiles(files, function (err, results) {
        ...
        _this.models.bin.latest({id: results.url, username: undefsafe(req, 'session.user.name') }, function (err, bin) {
          ...
          if (req.url.indexOf('edit') > -1) {
            ...
            _this.formatPreview(results, req, options, function (err, formatted) {
              if (err) {
                next(err);
              }
  ...
  formatPreview: function (bin, req, options, fn) {
    ...
    if (formatted.indexOf('%code%') > -1) {
      ...
      insert.push('<script id="jsbin-js">', bin.javascript.trim(), '</script>');
    }
    ...
    // Analytics
    // FIXME decide whether this should be included...
    ...
    // TODO December 6, 2013
    // - add helpers.render('previewHeader') and insert in document
    // - remove preview-header code
    helpers.render('preview-header', {
      username: undefsafe(req, 'session.user.name'),
      code_id_path: helpers.urlForBin(bin, true)
    }, function (err, header) {
      var re = null,
          lc = formatted.toLowerCase();
      if (lc.indexOf('<body') !== -1) {
        re = /<body[^>]*>/i;
      } else if (lc.indexOf('</head>') !== -1) {
        re = /<\/head>/i;
      } else {
        // slap it on the top
        re = /\n/;
      ...
      if (req.cookies.hide !== helpers.urlForBin(bin) + '/') {
        formatted = formatted.replace(re, function ($0) {
          return $0 + '\n' + (header || '').trim();
        });
      }
