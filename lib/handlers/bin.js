...
module.exports = Observable.extend({
  ...
  loadBinRevision: function (req, res, next) {
    if (req.param('rev')) {
      ...
      this.models.bin.latest({ id: req.bin.url, user: undefsafe(req, 'session.user') }, function(err, bin) {
        req.bin = bin;
        next();
      });
    }
  ...
  notFound: function (req, res, next) {
    ...
    if (req.isApi) {
      ...
      this.loadFiles(files, function (err, results) {
        ...
        _this.models.bin.latest({id: results.url, user: undefsafe(req, 'session.user') }, function (err, bin) {
          if (err) {
            return next(err);
          }
  ...
  loadBin: function (req, res, next) {
    var rev    = parseInt(req.params.rev, 10) || 1,
        query  = {id: req.params.bin, revision: rev, user: undefsafe(req, 'session.user')},
