...
function authorisedToViewBin(user, bin) {
  if (bin.visibility === 'private') {
    if (!user) {
      return false;
    } 
    if (user.name !== bin.name) {
      return false;
    }
  } 
  return true;
...
module.exports = Observable.extend({
  ...
  setBinAsPrivate: function(req, res, next) {
    ...
    this.models.bin.setBinVisibility(bin, 'private', function(err, bin){
      if (err) {
        res.send(500);
      }  
      res.send(200, 'OK');
  ...
  setBinAsPublic: function(req, res, next) {
    ...
    this.models.bin.setBinVisibility(bin, 'public', function(err, bin){
      if (err) {
        res.send(500);
      }  
      res.send(200, 'OK');
  ...
  createRevision: function (req, res, next) {
    ...
    if (req.param('method') === 'save') {
      ...
      params.summary = utils.titleForBin(params);
      params.visibility = req.bin.metadata.visibility;
  ...
  loadBin: function (req, res, next) {
    ...
    function complete(err, result) {
      ...
      if (!result) {
        ...
        models.bin.getBinMetadata(req.bin, function(err, binMetadata) {
          req.bin.metadata = binMetadata;
          next();
        });
      }
    }
  ...
  checkPrivate: function (req, res, next) {
    ...
    models.bin.getBinMetadata(req.bin, function(err, binMetadata){
      ...
      if (err) {
        req.bin = null;
        return next(new errors.NotFound('Could not find bin'));
      }

      // authorisedToViewBin is defined at the top of this file.
      if (!authorisedToViewBin(user, binMetadata)) {
        req.bin = null;
