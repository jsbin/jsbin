...
module.exports = Observable.extend({
  ...
  formatPreview: function (bin, options, fn) {
    ...
    function onAnalyticsComplete(err, analytics) {
      ...
      if (options.edit !== false) {
        var context = {root: helpers.url('', false, true), csrf: options.csrf};
        insert.push('<script src="' + helpers.url('js/render/edit.js', false, true) + '"></script>');
        insert.push('<script>jsbinShowEdit(' +  JSON.stringify(context) + ');</script>');
      }
  ...
  report: function (req, res, next) {
    ...
    this.models.bin.report(bin, function (err) {
      ...
      context = {
        url:  _this.helpers.url(req.url, true),
