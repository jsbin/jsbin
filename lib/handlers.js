  ...
  render: function (req, res, bin) {
    ...
    req.helpers.analytics(function (err, analytics) {
      res.render('index', {
        ...
        json_template: JSON.stringify(template).replace(/<\/script>/ig, '<\\/script>'),
  ...
  formatPreview: function (bin, helpers, options, fn) {
    ...
    if (formatted) {
      helpers.analytics(function (err, analytics) {
        ...
        if (options || options.spike === true) {
          insert.push('<script src="' + helpers.url('js/vendor/eventsource.js') + '"></script>');
          insert.push('<script src="' + helpers.url('js/spike.js') + '"></script>');
        }
