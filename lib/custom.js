...
if (config.whiteLabel) {
  ...
  var defaults = ['html', 'css', 'js'],
      root = config.customisation.base,
      extracted = {};
  ...
  try {
    ...
    defaults.forEach(function (ext) {
      ...
      if (extracted.defaults[key]) {
        if (extracted.defaults[key].indexOf('http') === 0) {
          // grab it from a url
        }
        return;
      }

      var uri = url.resolve(config.customisation.defaults.root, 'default.' + ext);
