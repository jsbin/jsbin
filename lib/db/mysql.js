...
module.exports = utils.inherit(Object, {
  ...
  updateBinData: function (bin, field, value, fn) {
    // let's do sanity check on valid fields
    if (templates.sandboxColumns.indexOf(field) === -1) {
      console.warn('Warning: attempt to update sandbox table with invalid field "' + field + '"');
      return fn(400); // bad request
    }

    var sql = templates.updateBinData.replace(':field', field);

    this.connection.query(sql, [value, bin.url, bin.revision], fn);
  },
  updateOwnersData: function (bin, field, value, fn) {
    // let's do sanity check on valid fields
    if (templates.ownersColumns.indexOf(field) === -1) {
      console.warn('Warning: attempt to update sandbox table with invalid field "' + field + '"');
      return fn(400); // bad request
    }

    var sql = templates.updateOwnersData.replace(':field', field);
