...
module.exports = utils.inherit(Object, {
  ...
  generateBinId: function (fn, attempts) {
    ...
    function tryurl(triesLeft, fn) {
      ...
      var id = utils.shortcode(3 + attempts);
    ...
    if (attempts <= 10) {
      tryurl(3, function (err, id) {
        if (err) {
          ...
          mysql.generateBinId(fn, attempts + 1);
        }
      });
    } else {
  ...
  isOwnerOf: function (params, fn) {
    ...
    this.connection.query(sql, values, function (err, result) {
      ...
      fn(null, { found: !!result.length, isowner: result.length ? result[0].owner === 1 : false, result: result });
    });
  },
  ...
  updateBin: function (bin, field, value, fn) {
    ...
    var sql = templates.updateBin.replace(':field', field);
