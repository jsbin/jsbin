...
module.exports = Observable.extend({
  constructor: function BinHandler(sandbox) {
    ...
    this.renderStack = [this.ensureBin, this.renderBin];
  },
  ...
  createRevisionOrClone: function (req, res, next) {
    ...
    if (req.param('method', '').indexOf('new') > -1 || !req.bin) {
    ...
    if (req.param('method') === 'save') {
      ...
      this.validateBin(params, function (err) {
        ...
        _this.models.user.isOwnerOf(username, params, function (err, result) {
          ...
          if (result.found && result.result[0].active === 'n') { // this is a deleted bin
  ...
  completeCreateBin: function (req, res, next) {
    var _this  = this,
        bin    = req.bin;
    ...
    if (req.session.user && req.session.user.name) {
      _this.models.user.setBinOwner(req.session.user.name, bin, function (err, result) {
        if (err) {
          return next(err);
        }
        render();
      ...
      this.loadFiles(files, function (err, results) {
        ...
        _this.models.bin.latest({id: results.url, username: undefsafe(req, 'session.user.name') }, function (err, bin) {
          ...
          if (req.url.indexOf('edit') > -1) {
            _this.renderBin(req, res, results);
          } else {
  ...
  loadBin: function (req, res, next) {
    ...
    function complete(err, result) {
      ...
      if (!result || err === 401) {
        // This is the switch for the ablity to create bins by typing the url.
        // Simply uncomment 'route' and any bins that don't exist get handled by
        // the not found error
        return next(/* 'route' */); 
      } else 
      if (!result.active) {
    ...
    // If the revision is latest or we are calling save, we want the latest bin.
    // Otherwise load the revision specified.
    var load = rev === 'latest' || req.url.indexOf('save') > -1 ? 'latest' : 'load';
  ...
  ensureBin: function(req, res, next) {
    if (req.bin) {
      return next();
    }

    // Here we can use any logic we like to load the default bins, for now I'm using a 
    // bin called default in the database, but we can use files or a req method such as
    // req.bin = req.getDefaultBin();

    this.models.bin.latest({id: 'default'}, function(err, result){
      req.bin = result;
      req.bin.url = req.param('bin');
      req.bin.revision = 1;
      next();
    });
  }, 
  renderBin: function (req, res) {
    var bin = req.bin;
  ...
  renderCreated: function (req, res, next) {
    var bin = req.bin,
        permalink = this.helpers.urlForBin(bin, true),
  ...
  delete: function (req, res, next) {
    ...
    var user = undefsafe(req, 'session.user.name'),
        owner = undefsafe(req, 'bin.metadata.name');

    if (user !== owner) {
      return res.send(403, {error: 'Not authorised.'});
    }

    // now archive the bin - FIXME https://github.com/remy/jsbin/issues/1056
    this.models.bin.archive({
      name: user,
      archive: true,
      url: req.bin.url,
      revision: req.bin.revision
    }, function () {});

    this.models.bin.updateBin(req.bin, 'active', 'n', function (error) {
      if (error) {
        console.error(error);
