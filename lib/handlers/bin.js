...
module.exports = Observable.extend({
  ...
  apiCreateBin: function (req, res, next) {
    ...
    this.validateBin(params, function (err) {
      ...
      bin.settings = bin.settings || '{ processors: {} }'; // set default processors
      bin.length = utils.binUrlLength(req);
      _this.models.bin.create(bin, function (err, result) {
        if (err) {
          return next(err);
  ...
  render: function renderEdit(req, res, bin) {
    this.protectVisibility(req.session.user, bin, function(err, bin) {
      ...
      var template = this.templateFromBin(bin),
          ...
          // sslForbin = features('sslForAll', { session: { user: { pro: !!undefsafe(req, 'bin.metadata.pro'), settings: { ssl: !!undefsafe(req, 'bin.metadata.pro') }}}}),
          sslForAll = features('sslForAll', req),
          ssl = req.secure && sslForAll,
      ...
      // IF you don't have the checksum AND the bin is currently writable (i.e. streaming)
      // AND you're not the owner - then make it readonly
      if ((!jsbin.state.checksum && jsbin.state.streaming === true) && (bin.metadata.name !== user.name) && bin.metadata.name !== 'anonymous') {
        jsbin.saveDisabled = true;
      }
  ...
  jsbin: function (bin, options) {
    ...
    return {
      ...
      state: {
        ...
        processors: undefsafe(bin, 'settings.processors') || {},
  ...
  loadFiles: function (files, fn) {
    ...
    if (!this.loadFiles.cache) {
      ...
      return fn(cache.error, cache.result);
    }
