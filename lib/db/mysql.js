...
MySQL.prototype = {
  ...
  getUserByForgotToken: function (token, fn) {
    ...
    this.connection.query(sql, [token, expires], function (err, results) {
      if (err) {
        return fn(err);
      }
      fn(null, results[0]);
  ...
  setForgotToken: function (user, token, fn) {
    ...
    this.connection.query(sql, [user, token], function (err, result) {
      if (err) {
        return fn(err);
      }
      fn(null, result.insertId);
  ...
  expireForgotToken: function (token, fn) {
    var sql = templates.setExpiredForgotToken,
        expires = this.expiredDate();
    ...
    this.connection.query(sql, [expires, token], function (err, results) {
      fn(err || null);
    });
  },
