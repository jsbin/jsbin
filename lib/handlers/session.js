...
module.exports = Observable.extend({
  ...
  dropboxCallback: function (req, res, next) {
    if (req.session.user) {
      req.session.user.dropbox_token = req.user.access_token;
    }
    res.flash(req.flash.NOTIFICATION, 'Wicked, your dropbox account has been linked to jsbin');
    res.redirect('/');
  },
