...
module.exports = function (app) {
  ...
  app.get('/upgrade', features.route('upgradeWithFeatures'), upgradeHandler.payment);

  app.post('/upgrade', features.route('upgradeWithFeatures'),
    /* login / reg */
    function(req, res, next){
      if (req.param('register')) {
        console.log('register');
        return userHandler.validateRegister(req, res, next);
      } else if (req.param('login')) {
        console.log('login');
        return userHandler.validateLogin(req, res, next);
      } else if (req.param('logged')) {
        console.log('logged');
        // return next('route');
        return next();
      } else {
    ...
    // 0 ? userHandler.validateLogin : userHandler.validateRegister,
