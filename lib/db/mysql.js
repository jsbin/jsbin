...
module.exports = utils.inherit(Object, {
  ...
  getUserByApiKey: function (apiKey, fn) {
    this.connection.query(templates.getUserByApiKey, [apiKey], fn);
  },
  getUserByEmail: function (email, fn) {
    this.connection.query(templates.getUserByEmail, [email], fn);
  },
  getUserByDomain: function (domain, fn) {
    this.connection.query(templates.getUserByDomain, [domain], fn);
  },
  ...
  touchLogin: function (id, fn) {
    var now = new Date();
    this.connection.query(templates.touchLogin, [now, id], function (err) {
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  updateUserDropboxData: function (id, token, fn) {
    var now = new Date();
    this.connection.query(templates.updateUserDropboxData, [token, now, id], function (err) {
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  expireForgotToken: function (token, fn) {
    ...
    this.connection.query(sql, [new Date(), token, null], function (err) {
      fn(err || null);
    });
  },
  expireForgotTokenByUser: function (user, fn) {
    ...
    this.connection.query(sql, [new Date(), null, user], function (err) {
      fn(err || null);
    });
  },
  expireDate: function () {
  ...
  reportBin: function (params, fn) {
    ...
    this.connection.query(sql, values, function (err) {
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  setCustomerActive: function (user, active, fn) {
    ...
    this.connection.query(sql, [active, user], function(err, result) {
      if (err) {
        return fn(err);
      }
      fn(null, result);
