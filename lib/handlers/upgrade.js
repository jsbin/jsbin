...
upgrade.updateCard = function (req, res, next) {
  ...
  getCustomerByUser(req.session.user).then(function (results) {
    var user = results[0];
    var expiry = req.body.expiry.split('/').map(function (s) {
      return parseInt(s.trim(), 10);
    });
    return stripe.customers.updateCard(user.stripe_id, req.body.id, {
      exp_month: expiry[0],
      exp_year: expiry[1]
    });
  }).then(function () {
...
upgrade.subscription = function (req, res, next) {
  ...
  getCustomerByUser(req.session.user).then(function (results) {
    ...
    var fail = require(__dirname + '/../../tmp/charge-fail.json');
    fail = { data: [ fail.data.object ]};
    var p2 = Promise.resolve(fail);
