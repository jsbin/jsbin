...
module.exports = Observable.extend({
  ...
  getCustom: function (req, res, next) {
    ...
    this.loadFiles(this.defaultFiles(), this.h, function (err, defaults) {
      if (err) {
        return next(err);
      }
  ...
  getBinPreview: function (req, res, next) {
    ...
    this.formatPreview(req.bin, this.h, options, function (err, formatted) {
      if (err) {
        next(err);
      }
  ...
  downloadBin: function (req, res, next) {
    ...
    this.formatPreview(bin, this.h, options, function (err, formatted) {
      if (err) {
        next(err);
      }
  ...
  render: function (req, res, bin, config) {
    ...
    if (req.session.user) {
      this.models.user.getBins(req.session.user.name, function (err, bins) {
        ...
        _this.formatHistory(bins, this.h, onComplete);
      });
    } else {
      onComplete();
    }
  ...
  renderFiles: function (req, res, files) {
    ...
    this.loadFiles(files, this.h, function (err, results) {
  ...
  renderCreated: function (req, res, bin) {
    var permalink = this.h.urlForBin(bin),
        editPermalink = this.h.editUrlForBin(bin);

    if (req.ajax) {
      if (req.param('format', '').toLowerCase() === 'plain') {
        return res.contentType('txt').send(this.h.editUrlForBin(bin, true));
      }

      res.json({
        code: bin.url,
        root: this.h.set('url full'),
      ...
      res.redirect(303, this.h.url(bin.url + '/' + bin.revision + '/edit'));
    }
  },
  renderHistory: function (req, res, bins) {
    this.formatHistory(bins, this.h, function (err, history) {
      res.send(history);
    });
  },
