...
module.exports = function (app) {
  ...
  app.get('/sign_s3', features.route('assets'), assetHandler.sign);
  ...
  var renderAccountSettings = (function(){
    var pages = ['editor', 'profile', 'delete', 'preferences', 'asset-manager'];
    var titles = {
    ...
    return function renderAccountSettings (req, res) {
      ...
      if (page === 'asset-manager' && !features('assets', req)) {
        page = false;
      }
  ...
  // 2014-10-06 - disabled in favour of real uploads to s3
  ...
  app.get('/:bin/:rev?/:quiet(quiet)?', ensureRevisionIsInt, secureOutput, sameoriginframes, binHandler.testPreviewAllowed, spike.getStream, binHandler.getBinPreview);
