 ...
 */
var process = function(data, settings) {
  data = data || {};
  settings = settings || {};
  ...
  if (!processors[processorName]) return data;
  // And process!
  data.content = processors[processorName](data.content);
  return data;
};
...
module.exports = {
  ...
  ping: function (bin, data) {
    ...
    pending[id].bin = bin;
    pending[id][data.panelId] = process(data, bin.settings);
