...
function authorisedToViewBin(user, bin) {
  return user.name === bin.name;
}
...
module.exports = Observable.extend({
  ...
  checkPrivate: function (req, res, next) {
    var user = req.session ? req.session.user : null,
        models = this.models;

    models.bin.getVisibility(req.bin, function(err, visibility){

      // If it's not private we're all good to continue;
      ...
      // If there's no user, they're not able to see a private bin;
      if (!user) {
        return next(new errors.NotFound('Pls sign in to see private bins'));
      }

      // This validates if user can see the bin. Currently it is based 
      // on whether they're they owner or not, however, this code should 
      // be moved and modified when we implement sharing private bins.
      models.user.isOwnerOf(user.name, req.bin, function(err, result) {
        if (err || !result.isowner) {
          return next(new errors.NotFound('you\'re not allowed to access this bin'));
        } else {
          next();  
        }
      });
