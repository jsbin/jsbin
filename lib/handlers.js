...
module.exports = handlers = {
  ...
  createBin: function (req, res, next) {
    ...
    handlers.validateBin(params, function (err) {
      if (err) {
        return next(err);
      }
      ...
      req.models.bin.create(params, function (err, result) {
        ...
        handlers.completeCreateBin(result, req, res, next);
      });
    });
  },
  ...
  completeCreateBin: function (bin, req, res, next) {
    var render = handlers.renderCreated.bind(handlers, req, res, bin);
  ...
  renderCreated: function (req, res, bin) {
    var permalink = req.helpers.urlForBin(bin, true),
        editPermalink = req.helpers.editUrlForBin(bin, true);

    if (req.ajax) {
      if (req.param('format', '').toLowerCase() === 'plain') {
        return res.contentType('txt').send(permalink);
      }
  ...
  formatHistory: function (bins, helpers, fn) {
    handlers.loadFiles(null, helpers, function (err, defaults) {
      var map = {}, data = [], key;
