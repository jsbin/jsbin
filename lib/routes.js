...
module.exports = function (app) {
  ...
  // Set up the routes.
  app.get('/', time('request.root'), function (req, res, next) {
    if (req.subdomain) {
      // test if this is a user with vanity feature
      sandbox.models.user.load(req.subdomain, function (error, user) {
        if (error) {
          return next('route');
        }
        ...
        if (features('vanity', { session: { user: user }})) {
          // do magic
          sandbox.models.user.getBookmark(user, 'vanity', function (err, data) {
            if (err) {
              console.log(err);
              return next('route');
            }
            ...
            binHandler.loadBin(req, res, function () {
              binHandler.getBinPreview(req, res, next);
            });
  ...
  app.get('/account/bookmark/vanity', features.route('vanity'), binParamFromReferer, binHandler.loadBin, function (req, res) {
    sandbox.models.user.saveBookmark(req.session.user, req.bin, 'vanity', function (err) {
      if (err) {
