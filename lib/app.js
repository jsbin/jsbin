...
if (options.url.runnerhost) {
  // strip trailing slash, just in case
  options.url.runnerhost = options.url.runnerhost.replace(/\/$/, '');
  app.set('url runnerhost', options.url.runnerhost);
} else {
  app.set('url runnerhost', app.set('url full'));
}
