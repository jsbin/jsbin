...
module.exports = Observable.extend({
  ...
  redirectUserAfterLogin: function (req, res) {
    var user = req.validatedUser;
    var referrer = req.sessionHandler.referer || req.param('referrer');
    if (req.ajax) {
      if(req.error) {
  ...
  updateUser: function (req, res, next) {
    ...
    // FIXME: convert below to promises - this is pre-promise crazy
  ...
  setUserSession: function (req, user) {
    ...
    req.session.user = {
      id: user.id,
      avatar: req.app.locals.gravatar(user),
      name: user.name,
      email: user.email,
