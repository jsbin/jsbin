...
MySQL.prototype = {
  ...
  updateUserEmail: function (id, email, fn) {
    var now = new Date();
    this.connection.query(templates.updateUserEmail, [email, now, id], function (err, results) {
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  updateUserKey: function (id, key, fn) {
    var now = new Date();
    this.connection.query(templates.updateUserKey, [key, now, id], function (err, results) {
      if (err) {
        return fn(err);
      }
      fn(null);
