...
module.exports = Observable.extend({
  ...
  getDefault: function (req, res, next) {
    if (req.subdomain && custom[req.subdomain]) {
      return this.getCustom(req, res, next, custom[req.subdomain]);
    } else if(config.custom && custom[config.custom]) {
      return this.getCustom(req, res, next, custom[config.custom]);
    }
  ...
  getCustom: function (req, res, next, config) {
    var overrides = config.defaults,
        _this = this;
      ...
      this.validateBin(bin, function (err) {
        ...
        handler.models.bin.updatePanel(panel, params, function (err, result) {
          ...
          if (req.session.user) {
            ...
            if (features('dropbox', req) && undefsafe(req, 'session.user.dropbox_token')) {
              handler.applyProcessors(bin).then(function (result) {
                models.bin.saveToDropbox(result[0], req.session.user);
              }).catch(function (error) {
                console.error(error);
              });
            }
  ...
  render: function (req, res, bin) {
    this.protectVisibility(req.session.user, bin, function(err, bin) {
      ...
      var template = this.templateFromBin(bin),
          customconfig = custom[req.subdomain] || custom[config.custom],
          helpers = this.helpers,
      ...
      helpers.renderAnalytics(function (err, analytics) {
