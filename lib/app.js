...
app.connect = function (callback) {
  ...
  app.configure(function () {
    ...
    app.use(function (req, res, next) {
      if (sessionVersion(req).lte('3.17.18')) {
        if (undefsafe(req, 'session.user.dropbox_token')) {
          var user = req.session.user;
          models.user.updateDropboxData(user.name, user.dropbox_token, function (err) { // jshint ignore: line
            if (err) {
              return next(err);
            }
            return next();
