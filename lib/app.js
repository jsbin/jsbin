...
/*
 var client = ldap.createClient({
   url: 'ldap://192.168.0.194:389'
 });

 var username = "jthomas";
 var password = "Jamesloves123*";
 var usernameField = "uid"
 var searchBase =  "dc=rotary-design,dc=com";
 var bindDN = "cn=admin,dc=rotary-design,dc=com";
 var bindPw = 'Yh1*32u1';
 client.bind(bindDN, bindPw, function(bindErr) {
  var opts = {
    //filter: '​(uid=jthomas)',
    scope: 'sub',
    attributes: ['dn', usernameField]
  };

  client.search(searchBase, opts, function(err, res) {
    //assert.ifError(err);
    if(err !== null) console.log(err, "ERROR")

    res.on('searchEntry', function(entry) {
      if(entry.object[usernameField] === username){
        console.log('entry: ' + JSON.stringify(entry.object));
        client.bind(entry.object.dn, password, function(bindErr2) {
          if(bindErr2 !== null) console.log(bindErr2);
        });
      }
    });
