...
module.exports = function (app) {
  ...
  var securePreviewFlag = app.get('security preview');
  var securePreviewURL = securePreviewFlag ? url.parse(securePreviewFlag) : null;
  var securePreview = function (req, res, next) {
    req.allowPreview = true;
    // return next();

    if (securePreviewFlag) {
      req.onSecurePreviewURL = req.headers.host === securePreviewURL.host;
      // if the referer is the main host, then allow the render
      if (req.onSecurePreviewURL && req.headers.referer) {
        req.allowPreview = url.parse(req.headers.referer).host === app.get('url host');
      } else {
        req.allowPreview = false;
      }

      if (!req.allowPreview && req.onSecurePreviewURL) {
        // redirect to securePreview url + path
        return res.redirect(app.get('url full') + req.url);
      }
    }
    next();
  };
  ...
  function renderLoginRegister(req, res) {
    ...
    res.render('register-login', {
      ...
      learn: 'http://jsbin.com' // TODO move to config setting
    });
  }
  ...
  app.get('/account/validate', function (req, res, next) {
    ...
    if (req.validated) {
      req.flash(req.flash.NOTIFICATION, 'Thank you for validating your email, you can now share full previews');
    } else {
      req.flash(req.flash.ERROR, 'Sorry, but your account has not been validated. Can you try again or <a href="' + app.get('urls issues') + '">report</a> this and include your email address?');
    }
    res.redirect(app.get('url full'));
  });
  ...
  // header
  app.get('/:bin/output-header', function (req, res) {
    res.render('output-header', {
      code_id_path: sandbox.helpers.urlForBin(req.bin, true),
    });
  });

  app.get('/:bin/:rev?/output-header', function (req, res) {
    res.render('output-header', {
      code_id_path: sandbox.helpers.urlForBin(req.bin, true),
    });
  });
  ...
  // Preview
  app.get('/:bin/:quiet(quiet)?', binHandler.testQuietAccess, spike.getStream, binHandler.getBinPreview);
  app.get('/:bin/:rev?/:quiet(quiet)?', binHandler.testQuietAccess, spike.getStream, binHandler.getBinPreview);
  ...
  app.get('/:bin',
    ...
    //       preview_root: app.get('security preview'),
    //       code_id_path: sandbox.helpers.urlForBin(req.bin)
    //     });
    //   }
