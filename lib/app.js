var nodemailer  = require('nodemailer'),
    ...
    versionTest = require('./session-version'),
...
app.connect = function (callback) {
  ...
  app.configure(function () {
    ...
    var oldSessionSecret = app.get('session secret');
    var currentSessionSecret = app.get('session secretNEW') || generateSessionSecret();
    var oldCookieParser = express.cookieParser(oldSessionSecret);
    var newCookieParser = express.cookieParser(currentSessionSecret);
    app.use(function (req, res, next) {
      function wrappedNext (err) {
        ...
        // console.log(req.signedCookies);
        if (req.signedCookies.jsbin) {
          console.log('we have signed cookies and we just used new parser');
          return next(err);
        } else {
          req.cookies = null;
          oldCookieParser(req, res, function(){
            req.secret = currentSessionSecret;
            console.log(req.signedCookies);
            if (req.signedCookies.jsbin) {
              console.log('we\'ve got an old cookie here!');
              console.log(req.signedCookies.jsbin);
              // TODO: This is where we need to redirect to login and flash a message etc
              // Current issues, redirect end up in endless loop because cookies don't get
              // updated to use latest secret, second issue is we don't have access to res.flash
              // this high up in the stack. We can't move it aboce this either as res.flash is
              // dependant on req.session, for which the middleware is below, and dependant on
              // this. AGHH!
              delete req.signedCookies.jsbin.user;

              res.flash(req.flash.NOTIFICATION, 'You have been logged out - WHHAHAAAAH<meta http-equiv="refresh" content="1;URL=/login">');
      ...
      return newCookieParser(req, res, wrappedNext);
    });
    app.use(express.cookieSession({
      key: 'jsbin',
      cookie: {
        maxAge: 365 * 24 * 60 * 60 * 1000,
        domain: app.get('url host').indexOf('.') === -1 ? undefined : '.' + app.get('url host')
      }
    }));
    app.use(function (req, res, next) {
