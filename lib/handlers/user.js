...
module.exports = Observable.extend({
  ...
  saveTemplateURL: function (req, res) {
    userModel.saveBookmark(req.session.user, req.bin, 'template', function (err) {
      if (err) {
        return res.send(500, err);
      }
      res.send({ ok: true });
  ...
  loadTemplateURL: function (req, res, next) {
    // userModel.getBookmark(user, 'template', function (err, data) {
    //   if (err || data.length === 0) {
    //     if (err) {
    //       console.error(err);
    //     }
