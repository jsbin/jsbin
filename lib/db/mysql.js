...
module.exports = utils.inherit(Object, {
  ...
  updateUserSettings: function(id, settings, fn) {
    this.connection.query(templates.updateUserSettings, [JSON.stringify(settings), id], function(err){
      if (err) {
        return fn(err);
      }
      fn(null);
  ...
  applyBinDefaults: function (bin) {
    ...
    if (!bin.last_updated || bin.last_updated == '0000-00-00 00:00:00' || isNaN(bin.last_updated.getTime())) bin.last_updated = new Date('2012-07-23 00:00:00');
  ...
  addProUserCount: function (username, fn) {
    ...
    pool.getConnection(function(error, conn) {
      ...
      conn.beginTransaction(function (error) {
        ...
        conn.query(templates.getNextProCount, function (error, result) {
          if (error) {
          ...
          var nextId = parseInt(result[0].value) + 1;

          this.insertCounter({ username: username, type: 'pro', value: nextId }, function (error) {
            if (error) {
              return conn.rollback(function() {
                fn(error);
              });
  ...
  insertCounter: function (fields, fn) {

    var columns = [];
    var values = [];
    var columnsArray = templates.countersColumns;

    Object.keys(fields).forEach(function(key) {
      if (columnsArray.indexOf(key) === -1) {
        throw new Error('Warning: attempt to update table with invalid field "' + key + '"');
      }
      columns.push(key);
      values.push(fields[key]);
    });

    var sql = templates.insertCounter
                .replace(':columns:', columns.join(', '))
                .replace(':values:', values.map(function () {
                  return '?';
                }).join(', '));

    this.connection.query(sql, values, fn);
  },
  getCounter: function (username, type, fn) {
    this.connection.query(templates.getCounter, [username, type], fn);
  },
});
