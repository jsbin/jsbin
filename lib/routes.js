...
module.exports = function (app) {
  ...
  app.post('/api/save', binHandler.apiCreateBin, binHandler.ensureBin, binHandler.renderCreated);
  app.post('/api/:bin/save', binHandler.ensureBin, binHandler.apiCreateRevision, binHandler.renderCreated);
  ...
  // This is called when I type into a blank url (jsbin.com/)
  app.post('/save', binHandler.createBin, binHandler.ensureBin, binHandler.completeCreateBin, binHandler.renderCreated);

  // This is called when I save in a named url (jsbin.com/mybin)
  app.post('/:bin/:rev?/save', binHandler.ensureBin, binHandler.createRevisionOrClone, binHandler.completeCreateBin, binHandler.renderCreated);

  /*
   * Bin State Changes
   */

  // <cmd> + <enter> reloads the bin on all watcher devices.
  app.post('/:bin/:rev?/reload', binHandler.reload);
  // delete a bin
  app.post('/:bin/:rev?/delete', features.route('delete'), binHandler.delete);
  // set a bin to private
  ...
  // Preview
  app.get('/:bin/:quiet(quiet)?', binHandler.ensureBin, spike.getStream, binHandler.getBinPreview);
  app.get('/:bin/:rev?/:quiet(quiet)?', binHandler.ensureBin, spike.getStream, binHandler.getBinPreview);
