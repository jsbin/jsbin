var postgresql = require('pg'),
    ...
    replaceWithReturnId = function($1) { return $1 + ' RETURNING id'; };
...
for (var id in sourceTemplates) {
  ...
  if (sourceTemplates.hasOwnProperty(id)) {
    ...
    templates[id] = sourceTemplates[id].
      ...
      replace(/`/g,'').
      replace(/^(INSERT INTO .*)$/, replaceWithReturnId); // get the ID for any inserted rows as a result
  }
}
  ...
  setBinPanel: function (panel, params, fn) {
    ...
    if (allowed[panel]) {
      this.client.query(sql, values, function (err, result) {
        ...
        that.getBin({ id: params.url, revision: params.revision }, function(err, result) {
          if (err) {
            return fn(err);
          }
          fn(null, result.id);
