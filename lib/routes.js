...
module.exports = function (app) {
  ...
  app.param('bin', function (req, res, next) {
    var binurl = req.params.bin.toLowerCase(),
        re = /[^\w\-]/;
    if (re.test(binurl) || app.settings.reserved.indexOf(binurl) !== -1) {
      return next(404);
    }
    next();
  }, binHandler.loadBin);
  ...
  app.get('/login', renderLoginRegister);
  app.get('/register', renderLoginRegister);
  ...
  app.post('/sethome', sessionHandler.checkUserLoggedIn, userHandler.validateLogin, sessionHandler.loginUser/*, sessionHandler.routeSetHome*/);
