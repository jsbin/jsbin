...
app.connect = function (callback) {
  ...
  app.configure(function () {
    ...
    var oldCookieParser = express.cookieParser('supersecret');
    var newCookieParser = express.cookieParser('newsupersecret');
    app.use(function (req, res, next){
      function wrappedNext (err) {
        console.log(req.cookies);
        if (!err && req.signedCookies.jsbin.user) {
          console.log('new session, all bless');
          console.log(req.signedCookies);
          return next();
        }
        console.log('old session');
        oldCookieParser(req, res, function(err) {
          if (err) {
            return next(err);
          }
          //res.flash(res.flash.NOTIFICATION, 'We\'ve updated security, please sign in again');
          console.log('security update');
          res.redirect('/login');
        });
