...
module.exports = {
  ...
  titleForBin: function (bin) {
    var html = (bin.html || '').replace(this.re.flatten, ' '), // flatten
        ...
        fixLength = function(str) { return str.substring(0,255-1); };

    // try to return the meta[description] first
    if (matches.length === 2 && matches[1].trim()) {
      return fixLength(matches[1]);
    }
    ...
    if (matches) {
      return fixLength(matches);
    }

    // No title return JavaScript.
    if (javascript) {
      return fixLength(javascript);
    }

    if (css) {
      return fixLength(css);
    }
    ...
    return fixLength(html.replace(this.re.tags, '').trim());
  },
  since: function (date) {
