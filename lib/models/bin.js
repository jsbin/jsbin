...
var model = {
  ...
  load: function (params, fn) {
    this.store.getBin(params, function (err, bin) {
      this.getBinMetadata(bin, function(err, binMetadata) {
        if (err) {
          return fn(err);
        }
  ...
  isVisible: function (bin, username) {
    ...
    console.log('isVisible:bin', bin);
    console.log('isVisible:user', username);
    if (bin && bin.active === 'y') {
