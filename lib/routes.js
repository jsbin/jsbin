...
module.exports = function (app) {
  ...
  app.get('/admin/*', noframes, function (req, res, next) {
    next('route');
  });
  ...
  if (features('sendy')) {
    app.get('/account/profile', sendy.middleware.setSubscribedOnUser, function (req, res, next) {
      next('route');
    });
  }
  ...
  // Admin
  app.get('/admin/*', features.route('admin'), function (req, res, next) {
    next('route');
  });

  var renderFlagBin = (function(){
    return function renderFlagBin (req, res) {
      var root = sandbox.helpers.url('', true, req.secure);
      var statik = sandbox.helpers.urlForStatic('', req.secure);

      res.render('admin/flag-bin', {
        title: 'Admin',
        layout: 'sub/layout.html',
        httproot: root.replace('https', 'http'),
        root: root,
  ...
  var renderFlagUser = (function(){
    return function renderFlagUser (req, res) {
      var root = sandbox.helpers.url('', true, req.secure);
      var statik = sandbox.helpers.urlForStatic('', req.secure);

      res.render('admin/flag-user', {
        title: 'Admin',
        layout: 'sub/layout.html',
        httproot: root.replace('https', 'http'),
        root: root,
  ...
  app.get('/admin/flag-bin', renderFlagBin);
  app.post('/admin/flag-bin', features.route('admin'), function(req, res) {
    var url = req.params.bin;
    var rev = req.params.rev;

    sandobx.models.bin.updateBinData({url: url, revision: rev}, {active: 'y'},  function(err) {
      if (err) {
        console.log(err.stack);
        res.send(400, err);
      }
  ...
  app.get('/admin/flag-user', renderFlagUser);
  app.post('/admin/flag-user', features.route('admin'), function(req, res) {
    var user = req.params.username;

    store.updateOwnershipData([user], {flagged: true},  function(err) {
      if (err) {
        console.log(err.stack);
        res.send(400, err);
      }
