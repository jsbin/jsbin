 ...
 */
utils.makeEvent = function (name, data) {
    ...
    if (sessionsFromUser[user]) {
      sessionsFromUser[user].forEach(function (oldbin) {
        ...
        if (oldbinKey !== key) {
          ...
          utils.removeConnection(oldbinKey, oldbin.res);

          session = utils.sessionForBin(bin, true);
  ...
  ping: function (bin, data) {
    ...
    pending[id].timer = setTimeout(function () {
      ...
      session.res.forEach(function (res) {
        ...
        res.write(utils.makeEvent(data.panelId, data.raw));
