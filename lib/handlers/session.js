...
module.exports = Observable.extend({
  ...
  githubCallback: function(req, res, next) {
    ...
    this.models.user.getOne('getUserByGithubId', [githubUser.profile.id], function (err, jsbinUser) {
      if (err) return this.respondWithMessage(res, res.flash.ERROR, 'Sorry, there was an error while accessing the database with your GitHub id.');
      // Was a user found?
      if (jsbinUser) {
        ...
        return this.models.user.updateGithubData(jsbinUser.name, jsbinUser.github_id, jsbinUser.github_token, function (err) {
          if (err) return this.respondWithMessage(res, res.flash.ERROR, 'Sorry, could not update your GitHub data & sign you in.');
          // Add the user data to the session.
          this.setUserSession(req, jsbinUser);
      ...
      if (sessionUser) {
        ...
        return this.models.user.updateGithubData(sessionUser.name, sessionUser.github_id, sessionUser.github_token, function (err) {
          if (err) return this.respondWithMessage(res, res.flash.ERROR, 'Sorry, could not connect your GitHub and JS Bin accounts.');
          // Add the user data to the session.
      ...
      this.models.user.load(githubUser.profile.username, function (err, jsbinUserFromGithubName) {
        if (err) return this.respondWithMessage(res, res.flash.ERROR, 'Sorry, there was an error while accessing the database with your GitHub username.');
        ...
        require('crypto').randomBytes(48, function(ex, buf) {
          ...
          this.models.user.create(userData, function (err, id) {
            if (err) return this.respondWithMessage(res, res.flash.ERROR, 'Sorry, there was an error while creating a new user.');
            // Add the user data to the session.
