...
upgrade.admin.reset = function (req, res, next) {
  if (!features('admin', req) || config.environment === 'production') {
    return next(403);
  }

  stripe.customers.list({ limit: 10 }).then(function (customers) {
    return Promise.all(customers.data.map(function (customer) {
  ...
  getCustomerByUser(req.session.user).then(function (results) {
    ...
    data.forEach(function (data) {
      if (data.plan.id.indexOf('vat') !== -1) {
        data.plan.vat = data.plan.amount * 0.8;
      }
    });
...
upgrade.cancel = function (req, res, next) {
  getCustomerByUser(req.session.user).then(function (results) {
    var result = results[0];
    if (debug) {console.log('.then, stripe.customers.retrieve(' + result.stripe_id + ')');} // jshint ignore:line
    return stripe.customers.cancelSubscription(result.stripe_id, req.body.subscription);// jshint ignore:line
...
upgrade.processPayment = function (req, res, next) {
  ...
  new Promise(function (resolve) {
    ...
    return getCustomerByUser(req.session.user).then(function (results) {
      ...
      req.flash(req.flash.NOTIFICATION, 'Welcome to the pro user lounge. Your seat is waiting. In the mean time, <a target="_blank" href="http://jsbin.com/help/pro">find out more about Pro accounts</a>');
      res.redirect('/');
    }).catch(function (error) {
