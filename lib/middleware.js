  ...
  api: function(options) {
    return function (req, res, next) {
      var apiKey,
          ...
          allowAnonymous = config.api.allowAnonymous,
          requireSSL = config.api.requireSSL;

      if (String(process.env.API_ALLOW_ANONYMOUS).toLowerCase() === 'true') {
        allowAnonymous = true;
      } else if (String(process.env.API_ALLOW_ANONYMOUS).toLowerCase() === 'false') {
        allowAnonymous = false;
      }

      if (String(process.env.API_REQUIRE_SSL).toLowerCase() === 'true') {
        requireSSL = true;
      } else if (String(process.env.API_REQUIRE_SSL).toLowerCase() === 'false') {
        requireSSL = false;
      }
      ...
      if (req.url.indexOf('/api') === 0) {
        ...
        if (allowAnonymous || (config.api.allowReadOnly && req.method === 'GET')) {
          next();
        } else {
          if (!apiKey) {
            res.status(403); // forbidden
