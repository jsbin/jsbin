...
var flags = {
  ...
  dropbox: false, // forced to remove 2021-10-17 // pro, // live June 20, 2014
  ...
  revisionless: true, // live June 13, 2015 - been trailing it for ages
  ...
  flagged: function (bin) {
    let created = new Date(bin.created);
    const now = Date.now();
    const oneHour = 60 * 60 * 1000;
    const oneDay = 24 * oneHour;

    // how old is the bin? allow 2 hours of sharing
    if (created.getTime() > (now - (2 * oneHour))) {
      return false;
    }

    if (!bin.metadata) { // anonymous
      // otherwise all anonymous bins can't be shared, but can be previewed
      // for the time being - 2023-08-10
      return true;
    }

    // first check if the account has been flagged
    if (undefsafe(bin, 'metadata.flagged')) {
      return true;
    }

    // if paid, then not flagged
    if (undefsafe(bin, 'metadata.pro')) {
      return false;
    }

    // if verified, then not flagged
    if (undefsafe(bin, 'metadata.verified')) {
      return false;
    }

    // otherwise, require 1 month old
    created = new Date(bin.metadata.created);

    if (created.getTime() < (now - (30 * oneDay))) {
      return false;
    }
