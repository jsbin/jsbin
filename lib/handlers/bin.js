...
module.exports = Observable.extend({
  ...
  formatPreview: function (bin, options, fn) {
    (function () {
      var processorName, processor, code, panel,
          processors = bin.settings && bin.settings.processors;

      if (processors) {
        for (panel in processors) {
          if (processors.hasOwnProperty(panel)) {
            processorName = processors[panel];
    ...
    function onAnalyticsComplete(err, analytics) {
      ...
      if (options.edit) {
        data = {root: helpers.urlForStatic(''), csrf: options.csrf};
