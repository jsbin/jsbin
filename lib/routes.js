...
module.exports = function (app) {
  ...
  function wrapWithFeature(feature, handler) {
    return function (req, res, next) {
      if (features(feature, req)) {
  ...
  app.get('/:bin/:quiet(quiet)?', secureOutput, wrapWithFeature('!pro', sameoriginframes), binHandler.testPreviewAllowed, spike.getStream, binHandler.getBinPreview);
  app.get('/:bin/:rev?/:quiet(quiet)?', ensureRevisionIsInt, secureOutput, wrapWithFeature('!pro', sameoriginframes), binHandler.testPreviewAllowed, spike.getStream, binHandler.getBinPreview);
  app.get('/:bin/:rev?/stats', tag('stats'), secureOutput, spike.getStream);

  app.post('/:bin/:rev/settings', features.route('metadata'), binHandler.ensureOwnership, binHandler.updateSettings);
  app.put('/:bin/:rev/settings', features.route('metadata'), binHandler.ensureOwnership, binHandler.updateSettings);
