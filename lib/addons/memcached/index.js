module.exports = function(app, connection) {
  ...
  app.use(function(req, res, next) {
    ...
    res.on('header', function () {
      if (req.session.user) {
        // FIXME - this is hacky
        if (JSON.stringify(undefsafe(req, '_sessionBeforeBlacklist.user') || {}) !== req._userJSONCopy) {
          memcached.set(req._sessionBeforeBlacklist.user.name, undefsafe(req, '_sessionBeforeBlacklist.user'));
        }
      }
    });
  });
