...
module.exports = Observable.extend({
  ...
  loginUser: function (req, res, next) {
    ...
    function onValidate(err, didPasswordMatch) {
      ...
      _this.respond(res, 200, {ok: true, key: '', created: false});
    }
  ...
  createUser: function (req, res, next) {
    ...
    this.models.user.create(params, function (err, id) {
      ...
      return this.respond(res, 200, {ok: true, key: '', created: true});
    });
  },
  updateUser: function (req, res, next) {
    ...
    if (req.user.name !== req.param('name')) {
      return this.respond(res, 400, {
        ok: false,
  ...
  resetPassword: function (req, res, next) {
    ...
    this.models.forgotToken.loadUser(token, function (err, user) {
      ...
      res.render('account', {
        email: user.email,
        action: _this.helpers.url('sethome'),
        csrf: req.session._csrf
      });
    });
  },
  ...
  respond: function (res, status, data) {
    if (res.ajax) {
      ...
      // Ideally need to pass a flash message here.
      res.redirect(this.helpers.url());
    }
  }
});
